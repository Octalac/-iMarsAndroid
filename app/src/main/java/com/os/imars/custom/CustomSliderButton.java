...
public class CustomSliderButton extends RelativeLayout {
    ...
    private float initialSlidingX = 0, x1 = 0, x2 = 0;
    private boolean moveLeftToRight = false;

    public CustomSliderButton(Context context) {
        this(context, null);
    }
    ...
    public CustomSliderButton(Context context, AttributeSet attrs, int defStyle) {
        ...
        setStuBackground(typedArray.getResourceId(R.styleable.StuButton_stu_background, R.drawable.bg_round_green_fill_color));
        setThumb(typedArray.getResourceId(R.styleable.StuButton_stu_thumbDrawable, R.drawable.bg_round_grey_shape));
    ...
    private void init(Context context) {
        ...
        inflater.inflate(R.layout.custom_buttton_slider, this, true);
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if (event.getAction() == MotionEvent.ACTION_DOWN) {
            ...
            moveLeftToRight = x2 > x1;
            float deltaX = x2 - x1;
            Log.e("deltaX : ", "deltaX : " + deltaX);

            if (deltaX != 0) {

                if (sliderPosition >= (getMeasuredWidth() - thumbWidth)) {
                    if (unlockListener != null) {
                        sliderPosition = getMeasuredHeight();
                        if (sliderPosition == getMeasuredHeight()) {
                            Log.d("1234", "onTouchEvent: " + getMeasuredHeight() + " postion" + sliderPosition);
        ...
        animator.addUpdateListener(valueAnimator -> {
            if (stuImgThumb != null) {
                if (moveLeftToRight) {
                    ...
                    params.leftMargin = stuBackground.getWidth() - stuImgThumb.getWidth();
                }
